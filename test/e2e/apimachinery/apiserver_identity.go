...
var _ = SIGDescribe("kube-apiserver identity", feature.APIServerIdentity, func() {
	...
	f.It("kube-apiserver identity should persist after restart", f.WithDisruptive(), f.WithProvider("gce"), func(ctx context.Context) {
		...
		framework.ExpectNoError(err, "failed to client.CoreV1.Nodes.List")
		...
		framework.ExpectNoError(err, "failed to client.CoordinationV1.Leases.List")
		...
		for _, node := range controlPlaneNodes {
			...
			framework.ExpectNoError(err, "failed to getControlPlaneHostname")
			...
			framework.ExpectNoError(err, "failed to client.CoordinationV1.Leases.Get")
			oldHolderIdentity := lease.Spec.HolderIdentity
			lastRenewedTime := lease.Spec.RenewTime

			err = restartAPIServer(ctx, &node)
			framework.ExpectNoError(err, "failed to restartAPIServer")
