...
var _ = SIGDescribe("SchedulerPredicates", framework.WithSerial(), func() {
	...
	ginkgo.AfterEach(func(ctx context.Context) {
		...
		if err == nil && *(rc.Spec.Replicas) != 0 {
			...
			framework.ExpectNoError(err, "failed to e2erc.DeleteRCAndWaitForGC")
		}
	})
	...
	f.It("validates local ephemeral storage resource limits of pods that are allowed to run", func(ctx context.Context) {
		...
		framework.ExpectNoError(err, "failed to cs.CoreV1.Pods.List")
	...
	ginkgo.Context("validates pod overhead is considered along with resource limits of pods that are allowed to run", func() {
		...
		ginkgo.It("verify pod overhead is accounted for", func(ctx context.Context) {
			...
			framework.ExpectNoError(e2epod.WaitForPodRunningInNamespace(ctx, cs, fillerPod), "failed to e2epod.WaitForPodRunningInNamespace")
	...
	*/
	framework.ConformanceIt("validates resource limits of pods that are allowed to run", func(ctx context.Context) {
		...
		for _, pod := range fillerPods {
			framework.ExpectNoError(e2epod.WaitForPodRunningInNamespace(ctx, cs, pod), "failed to e2epod.WaitForPodRunningInNamespace")
		}
	...
	*/
	framework.ConformanceIt("validates that NodeSelector is respected if matching", func(ctx context.Context) {
		...
		framework.ExpectNoError(e2epod.WaitForPodNotPending(ctx, cs, ns, labelPodName), "failed to e2epod.WaitForPodNotPending")
	...
	ginkgo.It("validates that taints-tolerations is respected if matching", func(ctx context.Context) {
		...
		// kubelet will then refuse to launch the pod.
		framework.ExpectNoError(e2epod.WaitForPodNotPending(ctx, cs, ns, tolerationPodName), "failed to e2epod.WaitForPodNotPending")
	...
	*/
	...
	ginkgo.It("validates Pods with non-empty schedulingGates are blocked on scheduling", func(ctx context.Context) {
		...
		framework.ExpectNoError(err, "failed to e2epod.WaitForNumberOfPods")
		framework.ExpectNoError(e2epod.WaitForPodsSchedulingGated(ctx, cs, ns, replicas, time.Minute), "failed to e2epod.WaitForPodsSchedulingGated")
		...
		for _, pod := range podList.Items {
			...
			framework.ExpectNoError(err, "failed to patchPod")
			pods = append(pods, live)
		}

		ginkgo.By("Expect all pods carry one scheduling gate and are still in pending state")
		framework.ExpectNoError(e2epod.WaitForPodsWithSchedulingGates(ctx, cs, ns, replicas, time.Minute, want), "failed to e2epod.WaitForPodsWithSchedulingGates")
		framework.ExpectNoError(e2epod.WaitForPodsSchedulingGated(ctx, cs, ns, replicas, time.Minute), "failed to e2epod.WaitForPodsSchedulingGated")
		...
		framework.ExpectNoError(e2epod.WaitForPodsRunning(ctx, cs, ns, replicas, time.Minute), "failed to e2epod.WaitForPodsRunning")
	})
	...
	ginkgo.It("when PVC has node-affinity to non-existent/illegal nodes, the pod should be scheduled normally if suitable nodes exist", func(ctx context.Context) {
		...
		framework.ExpectNoError(e2epod.WaitForPodNotPending(ctx, f.ClientSet, ns, podName), "failed to e2epod.WaitForPodNotPending")
	})
})
...
func runPausePod(ctx context.Context, f *framework.Framework, conf pausePodConfig) *v1.Pod {
	...
	framework.ExpectNoError(e2epod.WaitTimeoutForPodRunningInNamespace(ctx, f.ClientSet, pod.Name, pod.Namespace, f.Timeouts.PodStartShort), "failed to e2epod.WaitTimeoutForPodRunningInNamespace")
...
func WaitForSchedulerAfterAction(ctx context.Context, f *framework.Framework, action Action, ns, podName string, expectSuccess bool) {
	...
	framework.ExpectNoError(err, "failed to observeEventAfterAction")
...
func createHostPortPodOnNode(ctx context.Context, f *framework.Framework, podName, ns, hostIP string, port int32, protocol v1.Protocol, nodeSelector map[string]string, expectScheduled bool) {
	...
	if expectScheduled {
		framework.ExpectNoError(err, "failed to e2epod.WaitForPodNotPending")
	}
}
