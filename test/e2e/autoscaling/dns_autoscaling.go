...
var _ = SIGDescribe(feature.KubeDNSAutoscaler, "DNS horizontal autoscaling", func() {
	...
	ginkgo.BeforeEach(func(ctx context.Context) {
		...
		framework.ExpectNoError(err, "failed to getDNSReplicas")
		...
		framework.ExpectNoError(err, "failed to fetchDNSScalingConfigMap")
	...
	f.It(f.WithSerial(), f.WithSlow(), "kube-dns-autoscaler should scale kube-dns pods when cluster size changed", func(ctx context.Context) {
		...
		provider, err := detectDNSProvider(ctx, c)
		framework.ExpectNoError(err, "failed to detectDNSProvider")
		...
		framework.ExpectNoError(err, "failed to updateDNSScalingConfigMap")
		defer func() {
		...
		framework.ExpectNoError(err, "failed to waitForDNSReplicasSatisfied")
		...
		framework.ExpectNoError(err, "failed to WaitForClusterSizeFunc")
	...
	ginkgo.It("kube-dns-autoscaler should scale kube-dns pods in both nonfaulty and faulty scenarios", func(ctx context.Context) {
		...
		framework.ExpectNoError(err, "failed to deleteDNSScalingConfigMap")
		...
		framework.ExpectNoError(err, "failed to waitForDNSConfigMapCreated")
		...
		framework.ExpectNoError(err, "failed to deleteDNSAutoscalerPod")
