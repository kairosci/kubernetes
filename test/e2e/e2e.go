...
func setupSuite(ctx context.Context) {
	...
	framework.ExpectNoError(e2enode.WaitForAllNodesSchedulable(ctx, c, timeouts.NodeSchedulable), "failed to e2enode.WaitForAllNodesSchedulable")
...
func prepullImages(ctx context.Context, c clientset.Interface) {
	...
	framework.ExpectNoError(err, "failed to framework.CreateTestingNS")
	ns := namespace.Name
