...
func increaseClusterSizeWithTimeout(ctx context.Context, f *framework.Framework, c clientset.Interface, targetNodeCount int, timeout time.Duration) func() error {
	...
	framework.ExpectNoError(runAntiAffinityPods(ctx, f, f.Namespace.Name, targetNodeCount, "increase-size-pod", labels, labels), "failed to runAntiAffinityPods")
	...
	framework.ExpectNoError(waitForAllCaPodsReadyInNamespace(ctx, f, c), "failed to waitForAllCaPodsReadyInNamespace")
	return cleanupFunc
}
