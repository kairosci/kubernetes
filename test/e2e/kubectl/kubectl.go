...
func runKubectlRetryOrDie(ns string, args ...string) string {
	...
	framework.ExpectNoError(err, "failed to e2ekubectl.RunKubectl")
	return output
}
...
var _ = SIGDescribe("Kubectl client", func() {
	...
	ginkgo.Describe("Simple pod", func() {
		...
		ginkgo.BeforeEach(func(ctx context.Context) {
			...
			framework.ExpectNoError(e2epod.WaitTimeoutForPodReadyInNamespace(ctx, c, simplePodName, ns, framework.PodStartTimeout), "failed to e2epod.WaitTimeoutForPodReadyInNamespace")
		})
		...
		ginkgo.It("should support exec through kubectl proxy", func(ctx context.Context) {
			...
			framework.ExpectNoError(err, "failed to startProxyServer")
		...
		ginkgo.Context("should return command exit codes", func() {
			ginkgo.It("execing into a container with a successful command", func(ctx context.Context) {
				...
				framework.ExpectNoError(err, "failed to e2ekubectl.NewKubectlCommand.Exec")
			})
			...
			ginkgo.It("should handle in-cluster config", func(ctx context.Context) {
				...
				framework.ExpectNoError(err, "failed to os.MkdirTemp")
...
`), os.FileMode(0755)), "failed to os.MkdirTemp('', 'icc-override')")
		...
		ginkgo.Describe("Kubectl run", func() {
			ginkgo.It("running a successful command", func(ctx context.Context) {
				...
				framework.ExpectNoError(err, "failed to "+imageutils.GetE2EImage, "--restart=Never", podRunningTimeoutArg, "success", "--", "/bin/sh", "-c", "exit 0"")
			})
			...
			f.It(f.WithSlow(), "running a failing command without --restart=Never, but with --rm", func(ctx context.Context) {
				...
				framework.ExpectNoError(e2epod.WaitForPodNotFoundInNamespace(ctx, f.ClientSet, "failure-3", ns, 2*v1.DefaultTerminationGracePeriodSeconds*time.Second), "failed to e2epod.WaitForPodNotFoundInNamespace")
			})
			...
			f.It(f.WithSlow(), "running a failing command with --leave-stdin-open", func(ctx context.Context) {
				_, err := e2ekubectl.NewKubectlCommand(ns, "run", "-i", "--image="+imageutils.GetE2EImage(imageutils.BusyBox), "--restart=Never", podRunningTimeoutArg, "failure-4", "--leave-stdin-open", "--", "/bin/sh", "-c", "exit 42").
					WithStdinData("abcd1234").
					Exec()
				framework.ExpectNoError(err, "failed to "+imageutils.GetE2EImage, "--restart=Never", podRunningTimeoutArg, "failure-4", "--leave-stdin-open", "--", "/bin/sh", "-c", "exit 42"")
			})
		})
		...
		ginkgo.It("should support inline execution and attach", func(ctx context.Context) {
			...
			framework.ExpectNoError(c.CoreV1().Pods(ns).Delete(ctx, "run-test", metav1.DeleteOptions{}), "failed to c.CoreV1.Pods.Delete")
			...
			framework.ExpectNoError(c.CoreV1().Pods(ns).Delete(ctx, "run-test-2", metav1.DeleteOptions{}), "failed to c.CoreV1.Pods.Delete")
			...
			framework.ExpectNoError(err, "failed to run-test-3", 1*time.Minute, g")
			framework.ExpectNoError(e2epod.WaitTimeoutForPodReadyInNamespace(ctx, c, runTestPod.Name, ns, time.Minute), "failed to e2epod.WaitTimeoutForPodReadyInNamespace")

			framework.ExpectNoError(c.CoreV1().Pods(ns).Delete(ctx, "run-test-3", metav1.DeleteOptions{}), "failed to c.CoreV1.Pods.Delete")
		})
  ...
  "spec": {
	...
	ginkgo.Describe("Kubectl describe", func() {
		...
		*/
		...
		ginkgo.It("should check if kubectl describe prints relevant information for cronjob", func(ctx context.Context) {
			...
			framework.ExpectNoError(err, "failed to c.BatchV1.CronJobs.List")
		...
		*/
		framework.ConformanceIt("should create services for rc", func(ctx context.Context) {
			...
			validateService := func(name string, servicePort int, timeout time.Duration) {
				...
				framework.ExpectNoError(err, "failed to c.CoreV1.Services.Get")
			...
			framework.ExpectNoError(e2enetwork.WaitForService(ctx, c, ns, "rm2", true, framework.Poll, framework.ServiceStartTimeout), "failed to e2enetwork.WaitForService")
			...
			framework.ExpectNoError(e2enetwork.WaitForService(ctx, c, ns, "rm3", true, framework.Poll, framework.ServiceStartTimeout), "failed to e2enetwork.WaitForService")
	...
	ginkgo.Describe("Kubectl label", func() {
		...
		ginkgo.BeforeEach(func(ctx context.Context) {
			...
			e2ekubectl.RunKubectlOrDieInput(ns, podYaml, "create", "-f", "-")
			framework.ExpectNoError(e2epod.WaitTimeoutForPodReadyInNamespace(ctx, c, pausePodName, ns, framework.PodStartTimeout), "failed to e2epod.WaitTimeoutForPodReadyInNamespace")
		})
		ginkgo.AfterEach(func() {
	...
	ginkgo.Describe("Kubectl copy", func() {
		...
		ginkgo.BeforeEach(func(ctx context.Context) {
			...
			e2ekubectl.RunKubectlOrDieInput(ns, podYaml, "create", "-f", "-")
			framework.ExpectNoError(e2epod.WaitTimeoutForPodReadyInNamespace(ctx, c, busyboxPodName, ns, framework.PodStartTimeout), "failed to e2epod.WaitTimeoutForPodReadyInNamespace")
		})
		ginkgo.AfterEach(func() {
...
func validateGuestbookApp(ctx context.Context, c clientset.Interface, ns string) {
	...
	err := testutils.WaitForPodsWithLabelRunning(c, ns, label)
	framework.ExpectNoError(err, "failed to testutils.WaitForPodsWithLabelRunning")
...
func forEachReplicationController(ctx context.Context, c clientset.Interface, ns, selectorKey, selectorValue string, fn func(v1.ReplicationController)) {
	...
	for t := time.Now(); time.Since(t) < framework.PodListTimeout && ctx.Err() == nil; time.Sleep(framework.Poll) {
		...
		framework.ExpectNoError(err, "failed to c.CoreV1.ReplicationControllers.List")
		if len(rcs.Items) > 0 {
			break
		}
	}
