...
var _ = SIGDescribe("Multi-AZ Clusters", func() {
	...
	ginkgo.BeforeEach(func(ctx context.Context) {
		...
		if zoneCount <= 0 {
			...
			framework.ExpectNoError(err, "failed to e2enode.GetSchedulableClusterZones")
		...
		framework.ExpectNoError(err, "failed to createBalancedPodForNodes")
	})
...
func SpreadServiceOrFail(ctx context.Context, f *framework.Framework, replicaCount int, zoneNames sets.Set[string], image string) {
	...
	framework.ExpectNoError(testutils.StartPods(f.ClientSet, replicaCount, f.Namespace.Name, serviceName, *podSpec, false, framework.Logf), "failed to testutils.StartPods")
	...
	framework.ExpectNoError(err, "failed to e2epod.WaitForPodsWithLabelScheduled")
...
func checkZoneSpreading(ctx context.Context, c clientset.Interface, pods *v1.PodList, zoneNames []string) {
	...
	for _, pod := range pods.Items {
		...
		framework.ExpectNoError(err, "failed to getZoneNameForPod")
