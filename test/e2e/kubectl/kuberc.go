...
kind: Preference
`
			framework.ExpectNoError(os.WriteFile(kubercFile, []byte(minimalKuberc), os.FileMode(0644)), "failed to os.WriteFile")
		})
		...
		ginkgo.It("view should display kuberc file", func(ctx context.Context) {
			...
			kubercContent := fmt.Sprintf(kuberc, imageutils.GetE2EImage(imageutils.BusyBox), ns, ns)
			framework.ExpectNoError(os.WriteFile(kubercFile, []byte(kubercContent), os.FileMode(0755)), "failed to os.WriteFile")
