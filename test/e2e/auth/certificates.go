...
var _ = SIGDescribe("Certificates API [Privileged:ClusterAdmin]", func() {
	...
	*/
	ginkgo.It("should support building a client with a CSR", func(ctx context.Context) {
		...
		framework.ExpectNoError(err, "failed to cert.MakeCSR")
		...
		if err != nil {
			...
			defer func() {
				framework.ExpectNoError(f.ClientSet.RbacV1().ClusterRoleBindings().Delete(ctx, clusterRoleBinding.Name, metav1.DeleteOptions{}), "failed to f.ClientSet.RbacV1.ClusterRoleBindings.Delete")
			}()
		}

		framework.Logf("creating CSR")
		csr, err := csrClient.Create(ctx, csrTemplate, metav1.CreateOptions{})
		framework.ExpectNoError(err, "failed to csrClient.Create")
		defer func() {
			framework.ExpectNoError(csrClient.Delete(ctx, csr.Name, metav1.DeleteOptions{}), "failed to csrClient.Delete")
		}()
		...
		framework.ExpectNoError(err, "failed to cert.ParseCertsPEM")
		...
		framework.ExpectNoError(err, "failed to certificatesclient.NewForConfig")
		...
		framework.ExpectNoError(err, "failed to newClient.CertificateSigningRequests.Create")
		defer func() {
			framework.ExpectNoError(csrClient.Delete(ctx, newCSR.Name, metav1.DeleteOptions{}), "failed to csrClient.Delete")
		}()
	...
	*/
	framework.ConformanceIt("should support CSR API operations", func(ctx context.Context) {
		...
		framework.ExpectNoError(err, "failed to cert.GenerateSelfSignedCertKey")
		...
		framework.ExpectNoError(err, "failed to csrClient.Get")
		...
		framework.ExpectNoError(err, "failed to " + signerName}")
		...
		framework.ExpectNoError(err, "failed to " + createdCSR.Name}")
		...
		framework.ExpectNoError(err, "failed to csrClient.Patch")
		...
		framework.ExpectNoError(err, "failed to csrClient.Update")
		...
		framework.ExpectNoError(err, "failed to f.DynamicClient.Resource.Get")
		...
		framework.ExpectNoError(err, "failed to csrClient.UpdateApproval")
		...
		framework.ExpectNoError(err, "failed to csrClient.UpdateStatus")
		...
		framework.ExpectNoError(err, "failed to csrClient.Delete")
