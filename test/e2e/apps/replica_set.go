...
func testReplicaSetServeImageOrFail(ctx context.Context, f *framework.Framework, test string, image string) {
	...
	framework.ExpectNoError(e2epod.WaitForPodsResponding(ctx, f.ClientSet, f.Namespace.Name, name, labels.SelectorFromSet(rsLabels), true, 2*time.Minute, pods), "failed to e2epod.WaitForPodsResponding")
}
...
func listRSDeleteCollection(ctx context.Context, f *framework.Framework) {
	...
	framework.ExpectNoError(err, "failed to rsClient.Create")
